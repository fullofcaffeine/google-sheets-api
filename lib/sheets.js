...
Sheets.prototype.paintRow = function(documentId, sheetId, startRowIndex, endRowIndex, r, g, b) {
  var baseUrl = 'https://sheets.googleapis.com/v4/'
  var path =  'spreadsheets/' + documentId + ':batchUpdate'
  ...
  return this.post(path, baseUrl, body).then(function(data) {
    return data;
  });
}
...
Sheets.prototype.post = function post(path, overrideBaseUrl, body) {
  var self = this;
  return new Promise(function(resolve, reject) {
    self.authorize()
    .then(function(authClient) {
      request.post({
        url: (overrideBaseUrl || self.baseUrl) + path,
        auth: {
          bearer: authClient.credentials.access_token
        },
        json: true,
  ...
  return this.get(path).then(function(data) {
    return data.feed.entry.map(function(sheet) {
      ...
      return {
        ...
        v4id: sheet.link[2].href.split('gid=')[1],
